...
// POST /editar-simulado - Atualiza os dados de um simulado
router.post("/editar-simulado", verificarAutenticacao, async (req, res) => {
  try {
    const {
      id,
      professor_id,
...
// DELETE /deletar-simulado/:id - Deleta um simulado e suas associações
router.delete("/deletar-simulado/:id", verificarAutenticacao, (req, res) => {
  const db = req.db;
  const simuladoId = req.params.id;
  const professorId = req.session.user.id;

  // Deleta registros na tabela simulado_aluno
  db.query(
    `DELETE sa
    ...
    (err, result) => {
      ...
      // Deleta registros na tabela simulado_questao
      db.query(
        `DELETE sq
